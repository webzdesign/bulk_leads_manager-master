@extends('layouts.master')
@section('content')
                <div class="middleContent">
                    <div class="statsWrpr">
                        <div class="row statRow">
                            @if($leadTypes)
                                @foreach ($leadTypes  as $leadType )
                                @php
                                $leads = $leadType->leads->pluck('id');
                                $totalLeads = \App\Models\LeadDetail::whereIn('lead_id', $leads)->where('is_duplicate', 0)->where('is_invalid',0)->count();
                                 @endphp
                                <div class="col-md-6">
                                    <div class="statsCard">
                                        <div class="cards mb-0 position-relative">
                                            <div class="cardTop d-flex align-items-center justify-content-between">
                                                <div class="d-flex align-items-center">
                                                    <svg class="me-3" xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 34 34" fill="none">
                                                        <path d="M25.5 3.50625C23.0031 1.96562 20.1344 1.0625 17 1.0625V3.50625H25.5Z" fill="#ED4C5C"/>
                                                        <path d="M17 5.94961H28.475C27.5719 5.04648 26.5625 4.19648 25.5 3.50586H17V5.94961Z" fill="white"/>
                                                        <path d="M17 8.39395H30.4406C29.8562 7.49082 29.2188 6.69395 28.5281 5.9502H17V8.39395Z" fill="#ED4C5C"/>
                                                        <path d="M17 10.8373H31.7156C31.3438 9.9873 30.9187 9.1373 30.4406 8.39355H17V10.8373Z" fill="white"/>
                                                        <path d="M17 13.2816H32.5125C32.3 12.4316 32.0344 11.6348 31.7156 10.8379H17V13.2816Z" fill="#ED4C5C"/>
                                                        <path d="M17 15.7777H32.8844C32.8313 14.9277 32.6719 14.1309 32.5125 13.334H17V15.7777Z" fill="white"/>
                                                        <path d="M32.8844 15.7783H17V17.0002H1.0625C1.0625 17.4252 1.0625 17.7971 1.11562 18.2221H32.8844C32.9375 17.7971 32.9375 17.4252 32.9375 17.0002C32.9375 16.5752 32.9375 16.1502 32.8844 15.7783Z" fill="#ED4C5C"/>
                                                        <path d="M1.48748 20.6654H32.5125C32.725 19.8686 32.8312 19.0717 32.8844 18.2217H1.1156C1.16873 19.0186 1.3281 19.8686 1.48748 20.6654Z" fill="white"/>
                                                        <path d="M2.28442 23.1098H31.7157C32.0344 22.3129 32.3001 21.516 32.5126 20.666H1.48755C1.70005 21.516 1.96567 22.3129 2.28442 23.1098Z" fill="#ED4C5C"/>
                                                        <path d="M3.55942 25.5531H30.4407C30.9188 24.7562 31.3438 23.9594 31.7157 23.1094H2.28442C2.6563 23.9594 3.0813 24.7562 3.55942 25.5531Z" fill="white"/>
                                                        <path d="M5.47183 27.9965H28.5281C29.2187 27.2527 29.9093 26.4027 30.4406 25.5527H3.55933C4.09058 26.4559 4.7812 27.2527 5.47183 27.9965Z" fill="#ED4C5C"/>
                                                        <path d="M8.44692 30.4408H25.5532C26.6688 29.7502 27.625 28.9002 28.5282 27.9971H5.47192C6.37505 28.9533 7.38442 29.7502 8.44692 30.4408Z" fill="white"/>
                                                        <path d="M17 32.9373C20.1344 32.9373 23.0563 32.0342 25.5531 30.4404H8.4469C10.9438 32.0342 13.8656 32.9373 17 32.9373Z" fill="#ED4C5C"/>
                                                        <path d="M8.5 3.50625C7.38438 4.19687 6.375 5.04687 5.47188 5.95C4.72813 6.69375 4.09062 7.54375 3.55937 8.39375C3.08125 9.19063 2.60313 9.9875 2.28438 10.8375C1.96563 11.6344 1.7 12.4312 1.4875 13.2812C1.275 14.0781 1.16875 14.875 1.11562 15.725C1.0625 16.15 1.0625 16.575 1.0625 17H17V1.0625C13.8656 1.0625 10.9969 1.96562 8.5 3.50625Z" fill="#428BC1"/>
                                                        <path d="M13.2813 1.59375L13.5469 2.39062H14.3438L13.7063 2.92188L13.9188 3.71875L13.2813 3.24062L12.6438 3.71875L12.8563 2.92188L12.2188 2.39062H13.0156L13.2813 1.59375ZM15.4063 4.78125L15.6719 5.57812H16.4688L15.8313 6.10938L16.0438 6.90625L15.4063 6.42813L14.7688 6.90625L14.9813 6.10938L14.3438 5.57812H15.1406L15.4063 4.78125ZM11.1563 4.78125L11.4219 5.57812H12.2188L11.5813 6.10938L11.7938 6.90625L11.1563 6.42813L10.5188 6.90625L10.7313 6.10938L10.0938 5.57812H10.8906L11.1563 4.78125ZM13.2813 7.96875L13.5469 8.76562H14.3438L13.7063 9.29688L13.9188 10.0938L13.2813 9.61563L12.6438 10.0938L12.8563 9.29688L12.2188 8.76562H13.0156L13.2813 7.96875ZM9.03127 7.96875L9.2969 8.76562H10.0938L9.45627 9.29688L9.66878 10.0938L9.03127 9.61563L8.39377 10.0938L8.60628 9.29688L7.96877 8.76562H8.76565L9.03127 7.96875ZM4.78127 7.96875L5.0469 8.76562H5.84377L5.20627 9.29688L5.41877 10.0938L4.78127 9.61563L4.14377 10.0938L4.35627 9.29688L3.71877 8.76562H4.51565L4.78127 7.96875ZM15.4063 11.1562L15.6719 11.9531H16.4688L15.8313 12.4844L16.0438 13.2812L15.4063 12.8031L14.7688 13.2812L14.9813 12.4844L14.3438 11.9531H15.1406L15.4063 11.1562ZM11.1563 11.1562L11.4219 11.9531H12.2188L11.5813 12.4844L11.7938 13.2812L11.1563 12.8031L10.5188 13.2812L10.7313 12.4844L10.0938 11.9531H10.8906L11.1563 11.1562ZM6.90627 11.1562L7.1719 11.9531H7.96877L7.33127 12.4844L7.54377 13.2812L6.90627 12.8031L6.26877 13.2812L6.48127 12.4844L5.84377 11.9531H6.64065L6.90627 11.1562ZM13.2813 14.3438L13.5469 15.1406H14.3438L13.7063 15.6719L13.9188 16.4688L13.2813 15.9906L12.6438 16.4688L12.8563 15.6719L12.2188 15.1406H13.0156L13.2813 14.3438ZM9.03127 14.3438L9.2969 15.1406H10.0938L9.45627 15.6719L9.66878 16.4688L9.03127 15.9906L8.39377 16.4688L8.60628 15.6719L7.96877 15.1406H8.76565L9.03127 14.3438ZM4.78127 14.3438L5.0469 15.1406H5.84377L5.20627 15.6719L5.41877 16.4688L4.78127 15.9906L4.14377 16.4688L4.35627 15.6719L3.71877 15.1406H4.51565L4.78127 14.3438ZM6.26877 6.90625L6.90627 6.42813L7.54377 6.90625L7.27815 6.10938L7.91565 5.57812H7.11877L6.90627 4.78125L6.64065 5.57812H5.8969L6.5344 6.05625L6.26877 6.90625ZM2.01877 13.2812L2.65627 12.8031L3.29377 13.2812L3.02815 12.4844L3.66565 11.9531H2.9219L2.65627 11.1562L2.39065 11.9531H1.8594C1.8594 12.0063 1.80627 12.0594 1.80627 12.1125L2.23127 12.4312L2.01877 13.2812Z" fill="white"/>
                                                    </svg>
                                                    <div class="f-28 f-700 text-white">{{$leadType->name}}</div>
                                                </div>
                                                <div class="f-36 f-700 text-white">({{ $totalLeads }})</div>
                                            </div>
                                            <div class="cardYearList">
                                                @foreach ($leadType->ageGroup as $ageGroup)
                                                    <li class="d-flex align-items-center justify-content-between">
                                                        <div class="f-16 f-14-500 f-700 c-gr">{{ $ageGroup->age_from }} - {{ $ageGroup->age_to }} DAYS OLD</div>

                                                        <div class="f-16 f-14-500 f-700 c-gr">
                                                            {{ \App\Models\LeadDetail::whereIn('lead_id', $leads)->where('age_group_id', $ageGroup->id)->where('is_duplicate', 0)->where('is_invalid',0)->count() }}
                                                        </div>
                                                    </li>
                                                @endforeach

                                            </div>
                                        </div>
                                    </div>
                                </div>
                                @endforeach
                            @endif
                        </div>
                        <div class="row statRow statRow2">
                            <div class="col-md-6">
                                <div class="statsCard settingWrpr h-auto">
                                    <div class="cards mb-0 position-relative pb-0">
                                        <div class="cardTop d-flex align-items-center justify-content-center">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="33" height="34" viewBox="0 0 33 34" fill="none">
                                                <path d="M5.125 33.25C4.23125 33.25 3.46587 32.932 2.82887 32.2961C2.19296 31.6591 1.875 30.8937 1.875 30V11.6375C1.41458 11.3396 1.02892 10.9604 0.718 10.5C0.406 10.0396 0.25 9.49792 0.25 8.875V4C0.25 3.10625 0.5685 2.34087 1.2055 1.70387C1.84142 1.06796 2.60625 0.75 3.5 0.75H29.5C30.3937 0.75 31.1591 1.06796 31.7961 1.70387C32.432 2.34087 32.75 3.10625 32.75 4V8.875C32.75 9.49792 32.594 10.0396 32.282 10.5C31.9711 10.9604 31.5854 11.3396 31.125 11.6375V30C31.125 30.8937 30.807 31.6591 30.1711 32.2961C29.5341 32.932 28.7687 33.25 27.875 33.25H5.125ZM29.5 8.875V4H3.5V8.875H29.5ZM13.25 20.25H19.75C20.2104 20.25 20.5966 20.094 20.9086 19.782C21.2195 19.4711 21.375 19.0854 21.375 18.625C21.375 18.1646 21.2195 17.7784 20.9086 17.4664C20.5966 17.1555 20.2104 17 19.75 17H13.25C12.7896 17 12.4039 17.1555 12.093 17.4664C11.781 17.7784 11.625 18.1646 11.625 18.625C11.625 19.0854 11.781 19.4711 12.093 19.782C12.4039 20.094 12.7896 20.25 13.25 20.25Z" fill="white"/>
                                            </svg>
                                        </div>
                                        <div class="cardYearList text-end">
                                            <h4 class="c-7b f-20 f-500 mb-16 f-16-500">Leads Never Sent</h4>
                                            <h4 class="f-30 f-800 c-34 mb-0 f-20-500 mb-20-500">{{$inventory}}</h4>
                                        </div>

                                        <div class="cardsFooter f-16 f-700 c-e9">
                                            {{$inventeryPercent}}% <span class="c-7b f-300">than last week</span>
                                        </div>

                                    </div>
                                </div>
                                <div class="statsCard settingWrpr h-auto">
                                    <div class="cards mb-0 position-relative pb-0">
                                        <div class="cardTop d-flex align-items-center justify-content-center" style="background: linear-gradient(0deg, #FB8E03 0.08%, #FEA321 100.08%);">
                                            <svg width="44" height="34" viewBox="0 0 44 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M19.2439 33.3298C19.2439 33.3298 16.5602 33.3298 16.5602 30.5851C16.5602 27.8404 19.2439 19.6064 29.9787 19.6064C40.7134 19.6064 43.3971 27.8404 43.3971 30.5851C43.3971 33.3298 40.7134 33.3298 40.7134 33.3298H19.2439ZM29.9787 16.8617C32.1139 16.8617 34.1618 15.9942 35.6716 14.45C37.1815 12.9058 38.0297 10.8114 38.0297 8.62762C38.0297 6.44381 37.1815 4.34944 35.6716 2.80526C34.1618 1.26107 32.1139 0.393555 29.9787 0.393555C27.8434 0.393555 25.7955 1.26107 24.2857 2.80526C22.7758 4.34944 21.9276 6.44381 21.9276 8.62762C21.9276 10.8114 22.7758 12.9058 24.2857 14.45C25.7955 15.9942 27.8434 16.8617 29.9787 16.8617Z" fill="white"/>
                                                <path fill-rule="evenodd" clip-rule="evenodd" d="M14.4562 33.3291C14.0583 32.4723 13.8599 31.5328 13.8765 30.5844C13.8765 26.8654 15.7014 23.0365 19.0721 20.3742C17.3897 19.844 15.6368 19.5847 13.8765 19.6057C3.1417 19.6057 0.458008 27.8397 0.458008 30.5844C0.458008 33.3291 3.1417 33.3291 3.1417 33.3291H14.4562Z" fill="white"/>
                                                <path d="M12.5347 16.8631C14.3141 16.8631 16.0206 16.1402 17.2788 14.8533C18.5371 13.5665 19.2439 11.8212 19.2439 10.0014C19.2439 8.18153 18.5371 6.43622 17.2788 5.1494C16.0206 3.86258 14.3141 3.13965 12.5347 3.13965C10.7553 3.13965 9.04876 3.86258 7.79053 5.1494C6.5323 6.43622 5.82544 8.18153 5.82544 10.0014C5.82544 11.8212 6.5323 13.5665 7.79053 14.8533C9.04876 16.1402 10.7553 16.8631 12.5347 16.8631Z" fill="white"/>
                                            </svg>
                                        </div>
                                        <div class="cardYearList text-end">
                                            <h4 class="c-7b f-20 f-16-500 f-500 mb-16">Total Leads</h4>
                                            <h4 class="f-30 f-800 c-34 mb-0 f-20-500 mb-20-500">{{ $totalLeadsCount }}</h4>
                                        </div>

                                        <div class="cardsFooter f-16 f-700 c-e9">
                                            {{$leadsPercent}}% <span class="c-7b f-300">than last week</span>
                                        </div>

                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6 totalLead">
                                <div class="statsCard settingWrpr h-auto">
                                    <div class="cards mb-0 position-relative p-0">
                                        <div class="f-18-500 cardsHeader c-34 f-24 f-700" style="border-color: #4a24a947;">
                                            <svg class="me-3" xmlns="http://www.w3.org/2000/svg" width="45" height="45" viewBox="0 0 45 45" fill="none">
                                                <path d="M22.5 0C34.9268 0 45 10.0732 45 22.5C45 34.9268 34.9268 45 22.5 45C10.0732 45 0 34.9268 0 22.5C0 10.0732 10.0732 0 22.5 0ZM20.4075 12.5573C19.991 12.5573 19.5915 12.7227 19.297 13.0172C19.0025 13.3118 18.837 13.7112 18.837 14.1278V26.685C18.837 27.5535 19.539 28.2555 20.4075 28.2555H32.9648C33.1758 28.2635 33.3864 28.2288 33.5838 28.1536C33.7811 28.0783 33.9613 27.964 34.1135 27.8175C34.2657 27.6709 34.3867 27.4952 34.4694 27.3008C34.5521 27.1064 34.5947 26.8974 34.5947 26.6861C34.5947 26.4749 34.5521 26.2658 34.4694 26.0714C34.3867 25.877 34.2657 25.7013 34.1135 25.5548C33.9613 25.4083 33.7811 25.294 33.5838 25.2187C33.3864 25.1434 33.1758 25.1088 32.9648 25.1168H21.9758V14.1278C21.9758 13.7116 21.8106 13.3125 21.5166 13.018C21.2225 12.7236 20.8236 12.5578 20.4075 12.5573Z" fill="url(#paint0_linear_1524_278075)"/>
                                                <defs>
                                                <linearGradient id="paint0_linear_1524_278075" x1="22.5" y1="0" x2="22.5" y2="45" gradientUnits="userSpaceOnUse">
                                                <stop stop-color="#63B967"/>
                                                <stop offset="1" stop-color="#4BA64F"/>
                                                </linearGradient>
                                                </defs>
                                            </svg>
                                            Total Leads Imported
                                        </div>
                                        <div class="cardYearList">
                                            <div class="d-flex align-items-center justify-content-between">
                                                <div class="f-20 f-16-500 f-700 c-fb d-flex align-items-center">
                                                    <svg class="me-2" xmlns="http://www.w3.org/2000/svg" width="18" height="20" viewBox="0 0 18 20" fill="none">
                                                        <path d="M9 0.5C13.9707 0.5 18 4.75315 18 10C18 15.2469 13.9707 19.5 9 19.5C4.0293 19.5 0 15.2469 0 10C0 4.75315 4.0293 0.5 9 0.5ZM8.163 5.80195C7.99639 5.80195 7.83661 5.87181 7.7188 5.99617C7.60099 6.12052 7.5348 6.28919 7.5348 6.46505V11.767C7.5348 12.1337 7.8156 12.4301 8.163 12.4301H13.1859C13.2703 12.4335 13.3546 12.4188 13.4335 12.3871C13.5125 12.3553 13.5845 12.307 13.6454 12.2451C13.7063 12.1833 13.7547 12.1091 13.7878 12.027C13.8208 11.9449 13.8379 11.8567 13.8379 11.7675C13.8379 11.6783 13.8208 11.59 13.7878 11.5079C13.7547 11.4259 13.7063 11.3517 13.6454 11.2898C13.5845 11.2279 13.5125 11.1797 13.4335 11.1479C13.3546 11.1161 13.2703 11.1015 13.1859 11.1049H8.7903V6.46505C8.7903 6.28935 8.72424 6.12083 8.60662 5.9965C8.48901 5.87218 8.32945 5.8022 8.163 5.80195Z" fill="url(#paint0_linear_1524_278085)"/>
                                                        <defs>
                                                        <linearGradient id="paint0_linear_1524_278085" x1="9.00847" y1="19.4855" x2="9.00847" y2="0.485512" gradientUnits="userSpaceOnUse">
                                                        <stop stop-color="#FB8E03"/>
                                                        <stop offset="1" stop-color="#FEA321"/>
                                                        </linearGradient>
                                                        </defs>
                                                    </svg>
                                                    Last 24 hours
                                                </div>
                                                <div class="c-34 f-24 f-18-500 f-700">@if($last24hours){{$last24hours}} @else 0 @endif</div>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <div class="f-20 f-16-500 f-700 c-43 d-flex align-items-center">
                                                    <svg class="me-2" width="18" height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M9 0.5C13.9707 0.5 18 4.75315 18 10C18 15.2469 13.9707 19.5 9 19.5C4.0293 19.5 0 15.2469 0 10C0 4.75315 4.0293 0.5 9 0.5ZM8.163 5.80195C7.99639 5.80195 7.83661 5.87181 7.7188 5.99617C7.60099 6.12052 7.5348 6.28919 7.5348 6.46505V11.767C7.5348 12.1337 7.8156 12.4301 8.163 12.4301H13.1859C13.2703 12.4335 13.3546 12.4188 13.4335 12.3871C13.5125 12.3553 13.5845 12.307 13.6454 12.2451C13.7063 12.1833 13.7547 12.1091 13.7878 12.027C13.8208 11.9449 13.8379 11.8567 13.8379 11.7675C13.8379 11.6783 13.8208 11.59 13.7878 11.5079C13.7547 11.4259 13.7063 11.3517 13.6454 11.2898C13.5845 11.2279 13.5125 11.1797 13.4335 11.1479C13.3546 11.1161 13.2703 11.1015 13.1859 11.1049H8.7903V6.46505C8.7903 6.28935 8.72424 6.12083 8.60662 5.9965C8.48901 5.87218 8.32945 5.8022 8.163 5.80195Z" fill="url(#paint0_linear_1524_278101)"/>
                                                        <defs>
                                                        <linearGradient id="paint0_linear_1524_278101" x1="9" y1="0.5" x2="9" y2="19.5" gradientUnits="userSpaceOnUse">
                                                        <stop stop-color="#439DEE"/>
                                                        <stop offset="1" stop-color="#1E78E9"/>
                                                        </linearGradient>
                                                        </defs>
                                                    </svg>
                                                    Last 7 days
                                                </div>
                                                <div class="c-34 f-24 f-18-500 f-700">@if($last7days)
                                                        {{$last7days}}@else 0
                                                @endif</div>
                                            </div>
                                            <div class="d-flex align-items-center justify-content-between">
                                                <div class="f-20 f-16-500 f-700 c-43 d-flex align-items-center">
                                                    <svg class="me-2" width="18" height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M9 0.5C13.9707 0.5 18 4.75315 18 10C18 15.2469 13.9707 19.5 9 19.5C4.0293 19.5 0 15.2469 0 10C0 4.75315 4.0293 0.5 9 0.5ZM8.163 5.80195C7.99639 5.80195 7.83661 5.87181 7.7188 5.99617C7.60099 6.12052 7.5348 6.28919 7.5348 6.46505V11.767C7.5348 12.1337 7.8156 12.4301 8.163 12.4301H13.1859C13.2703 12.4335 13.3546 12.4188 13.4335 12.3871C13.5125 12.3553 13.5845 12.307 13.6454 12.2451C13.7063 12.1833 13.7547 12.1091 13.7878 12.027C13.8208 11.9449 13.8379 11.8567 13.8379 11.7675C13.8379 11.6783 13.8208 11.59 13.7878 11.5079C13.7547 11.4259 13.7063 11.3517 13.6454 11.2898C13.5845 11.2279 13.5125 11.1797 13.4335 11.1479C13.3546 11.1161 13.2703 11.1015 13.1859 11.1049H8.7903V6.46505C8.7903 6.28935 8.72424 6.12083 8.60662 5.9965C8.48901 5.87218 8.32945 5.8022 8.163 5.80195Z" fill="url(#paint0_linear_1524_278096)"/>
                                                        <defs>
                                                        <linearGradient id="paint0_linear_1524_278096" x1="9" y1="0.5" x2="9" y2="19.5" gradientUnits="userSpaceOnUse">
                                                        <stop stop-color="#3F189F"/>
                                                        <stop offset="1" stop-color="#4C26AA"/>
                                                        </linearGradient>
                                                        </defs>
                                                    </svg>
                                                    Last 30 days
                                                </div>
                                                <div class="c-34 f-24 f-18-500 f-700">@if($last30days)
                                                    {{$last30days}} @else 0
                                            @endif</div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
    @endsection

    @section('script')
    <script>

    </script>
    @endsection
